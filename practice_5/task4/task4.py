...
def select_top(collection, year):
    ...
    result = list(cursor)
    with open(f'./results/select_queries/top_{year}.json', 'w', encoding='utf-8') as file:
        file.write(dumps(result, ensure_ascii=False))

def select_by_sex(collection):
    cursor = collection.find({'sex': 'Female'})
    result = list(cursor)
    with open('./results/select_queries/select_by_sex.json', 'w', encoding='utf-8') as file:
        file.write(dumps(result, ensure_ascii=False))

def select_by_country(collection):
    cursor = collection.find({'country_code': {'$in': ['RO', 'CZ', 'DE', 'FR']}})
    result = list(cursor)
    with open('./results/select_queries/select_by_country.json', 'w', encoding='utf-8') as file:
        file.write(dumps(result, ensure_ascii=False))
...
def complex_selection(collection):
    ...
    result = list(cursor)
    with open('./results/select_queries/complex_selection.json', 'w', encoding='utf-8') as file:
        file.write(dumps(result, ensure_ascii=False))
...
def count_documents(collection):
    ...
    result = list(cursor)
    with open(f'./results/aggregation_queries/count_documents.json', 'w', encoding='utf-8') as file:
        file.write(dumps(result, ensure_ascii=False))
...
def get_hly_stat(collection, year):
    ...
    cursor = collection.aggregate(query)
    result = list(cursor)
    with open(f'./results/aggregation_queries/hly_stat_{year}.json', 'w', encoding='utf-8') as file:
        file.write(dumps(result, ensure_ascii=False))
...
def get_hly_stat_by_sex(collection, year):
    ...
    cursor = collection.aggregate(query)
    result = list(cursor)
    with open(f'./results/aggregation_queries/hly_stat_by_sex_{year}.json', 'w', encoding='utf-8') as file:
        file.write(dumps(result, ensure_ascii=False))
...
def update_by_country(collection, year, country_code):
    ...
    print(f"Count of modified documents (increment data in {year}, country = {country_code}): ", result.modified_count)
...
def update_complex(collection):
    ...
    print(f"Count of modified documents (by complex predicate): ", result.modified_count)
